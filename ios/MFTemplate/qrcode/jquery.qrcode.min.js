	...
	u.fn.qrcode = function(l) {
		...
		p.prototype = {
			...
			make : function() {
				if (this.typeNumber < 1) {
					var a = 1;
					for (a = 1; a < 40; a++) {
						for ( var c = q.getRSBlocks(a, this.errorCorrectLevel), d = new w, b = 0, e = 0; e < c.length; e++)
							b += c[e].dataCount;
			...
			setupTimingPattern : function() {
				...
				for (a = 8; a < this.moduleCount - 8; a++)
					if (this.modules[6][a] == null)
						this.modules[6][a] = a % 2 == 0
			},
			...
			setupTypeNumber : function(a) {
				...
				for (d = 0; d < 18; d++) {
					b = !a && (c >> d & 1) == 1;
					this.modules[d % 3 + this.moduleCount - 8 - 3][Math
							.floor(d / 3)] = b
				}
			},
			...
			mapData : function(a, c) {
				for ( var d = -1, b = this.moduleCount - 1, e = 7, f = 0, h = this.moduleCount - 1; h > 0; h -= 2)
					for (h == 6 && h--;;) {
						for ( var g = 0; g < 2; g++)
							if (this.modules[b][h - g] == null) {
								var k = false;
								if (f < a.length)
									k = (a[f] >>> e & 1) == 1;
								if (j.getMask(c, b, h - g))
									k = !k;
								this.modules[b][h - g] = k;
								e--;
								if (e == -1) {
									f++;
									e = 7
		...
		p.createData = function(a, c, d) {
			...
			for (e = a = 0; e < c.length; e++)
				a += c[e].dataCount;
			if (b.getLengthInBits() > a * 8)
				throw Error("code length overflow. (" + b.getLengthInBits()
						+ ">" + a * 8 + ")");
			for (b.getLengthInBits() + 4 <= a * 8 && b.put(0, 4); b
					.getLengthInBits() % 8 != 0;)
				b.putBit(false);
			for (;;) {
				if (b.getLengthInBits() >= a * 8)
		...
		for ( var o = {
			...
			getBCHTypeInfo : function(a) {
				for ( var c = a << 10; j.getBCHDigit(c) - j.getBCHDigit(j.G15) >= 0;)
					c ^= j.G15 << j.getBCHDigit(c) - j.getBCHDigit(j.G15);
				return (a << 10 | c) ^ j.G15_MASK
			},
			getBCHTypeNumber : function(a) {
				for ( var c = a << 12; j.getBCHDigit(c) - j.getBCHDigit(j.G18) >= 0;)
			...
			getErrorCorrectPolynomial : function(a) {
				for ( var c = new t([ 1 ], 0), d = 0; d < a; d++)
					c = c.multiply(new t([ 1, m.gexp(d) ], 0));
				return c
			},
			...
			getLostPoint : function(a) {
				for ( var c = a.getModuleCount(), d = 0, b = 0; b < c; b++)
					for ( var e = 0; e < c; e++) {
						for ( var f = 0, h = a.isDark(b, e), g = -1; g <= 1; g++)
							if (!(b + g < 0 || c <= b + g))
								for ( var k = -1; k <= 1; k++)
									e + k < 0 || c <= e + k || g == 0 && k == 0
											|| h == a.isDark(b + g, e + k)
				...
				for (b = 0; b < c; b++)
					for (e = 0; e < c - 6; e++)
						if (a.isDark(b, e) && !a.isDark(b, e + 1)
				...
				for (e = 0; e < c; e++)
					for (b = 0; b < c - 6; b++)
						if (a.isDark(b, e) && !a.isDark(b + 1, e)
			...
			m.EXP_TABLE[n] = 1 << n;
		for (n = 8; n < 256; n++)
			m.EXP_TABLE[n] = m.EXP_TABLE[n - 4] ^ m.EXP_TABLE[n - 5]
					^ m.EXP_TABLE[n - 6] ^ m.EXP_TABLE[n - 8];
		for (n = 0; n < 255; n++)
			...
			multiply : function(a) {
				for ( var c = Array(this.getLength() + a.getLength() - 1), d = 0; d < this
					...
					for ( var b = 0; b < a.getLength(); b++)
						c[d + b] ^= m.gexp(m.glog(this.get(d))
								+ m.glog(a.get(b)));
				return new t(c, 0)
			},
			mod : function(a) {
				if (this.getLength() - a.getLength() < 0)
					return this;
				for ( var c = m.glog(this.get(0)) - m.glog(a.get(0)), d = Array(this
						.getLength()), b = 0; b < this.getLength(); b++)
					d[b] = this.get(b);
				for (b = 0; b < a.getLength(); b++)
					d[b] ^= m.gexp(m.glog(a.get(b)) + c);
				return (new t(d, 0)).mod(a)
			}
		};
		q.RS_BLOCK_TABLE = [ [ 1, 26, 19 ], [ 1, 26, 16 ], [ 1, 26, 13 ],
				...
				[ 6, 43, 15, 2, 44, 16 ], [ 4, 101, 81 ],
				[ 1, 80, 50, 4, 81, 51 ], [ 4, 50, 22, 4, 51, 23 ],
				[ 3, 36, 12, 8, 37, 13 ], [ 2, 116, 92, 2, 117, 93 ],
				[ 6, 58, 36, 2, 59, 37 ], [ 4, 46, 20, 6, 47, 21 ],
				[ 7, 42, 14, 4, 43, 15 ], [ 4, 133, 107 ],
				...
				[ 5, 65, 41, 5, 66, 42 ], [ 5, 54, 24, 7, 55, 25 ],
				[ 11, 36, 12 ], [ 5, 122, 98, 1, 123, 99 ],
				...
				[ 17, 68, 42 ], [ 17, 50, 22, 6, 51, 23 ],
				[ 19, 46, 16, 6, 47, 17 ], [ 2, 139, 111, 7, 140, 112 ],
				[ 17, 74, 46 ], [ 7, 54, 24, 16, 55, 25 ], [ 34, 37, 13 ],
		...
		q.getRSBlocks = function(a, c) {
			...
			for ( var b = d.length / 3, e = [], f = 0; f < b; f++)
				for ( var h = d[f * 3 + 0], g = d[f * 3 + 1], k = d[f * 3 + 2], r = 0; r < h; r++)
					e.push(new q(g, k));
			return e
		};
		...
		w.prototype = {
			...
			putBit : function(a) {
				...
				if (a)
					this.buffer[c] |= 128 >>> this.length % 8;
				this.length++
			}
		};
		if (typeof l === "string")
		...
		return this
				.each(function() {
					...
					if (l.render == "canvas") {
						...
						c.width = l.width;
						c.height = l.height;
						for ( var d = c.getContext("2d"), b = l.width
								/ a.getModuleCount(), e = l.height
								/ a.getModuleCount(), f = 0; f < a
								.getModuleCount(); f++)
							for ( var h = 0; h < a.getModuleCount(); h++) {
								d.fillStyle = a.isDark(f, h) ? "#000000"
										: "#ffffff";
						...
						// Compatibilité IE7 : IE7 ne tient pas compte des décimales de pixels. Les dernières lignes et colonnes sont donc étirées 
						// pour remplir le container de 256x256 => déformation de l'image => scan KO. Anomalie non présente sur IE6, (IE8 ?), FF.
						// Solution : on calcule les dimensions du container de sorte que chaque cellule ait des hauteur et largeur entières.
						var cellSize = 4; // arbitraire
						var tableSize = a.getModuleCount() * cellSize;
						
						c = u("<table></table>").css("width", tableSize + "px")
								.css("height", tableSize + "px").css("border",
										"0px").css("border-collapse",
										"collapse").css("background-color",
										"#ffffff");
						d = 100 / a.getModuleCount();
						b = 100 / a.getModuleCount();
						for (e = 0; e < a.getModuleCount(); e++) {
							f = u("<tr></tr>").css("height", b + "%").appendTo(
									c);
							for (h = 0; h < a.getModuleCount(); h++)
								u("<td></td>").css("width", d + "%").css(
										"background-color",
										a.isDark(e, h) ? "#000000" : "#ffffff")
										.appendTo(f)
						}
						a = c
					}
